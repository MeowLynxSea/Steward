...
//! "andrew", "grace", "household"), identity/config files (AGENTS.md,
//! SOUL.md, TOOLS.md, BOOTSTRAP.md) must ONLY come from the primary scope.
//! Silently inheriting identity from another scope is a correctness and
//! security issue — the agent would present itself as the wrong user.
//!
...
//! 3. All prompt/config files remain scope-isolated
#![cfg(feature = "libsql")]
...
use steward_core::workspace::{Workspace, paths};
...
// ─── Test 3: All identity files are scope-isolated ──────────────────────
...
async fn all_identity_files_are_scope_isolated() {
    ...
    assert!(
        ...
        "No identity or config files from secondary scope should appear.\n\
         Every identity/config file (AGENTS.md, SOUL.md, BOOTSTRAP.md, \
         TOOLS.md) must read from primary scope only.\nPrompt:\n{prompt}"
    );
}
