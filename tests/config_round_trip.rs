...
//! functions from `steward_core::bootstrap`, ensuring test coverage of the actual
//! escaping/formatting logic rather than a reimplementation.

use std::collections::HashMap;
use tempfile::tempdir;

use steward_core::bootstrap::{save_bootstrap_env_to, upsert_bootstrap_var_to};
...
fn bootstrap_env_preserves_existing_values() {
    ...
    assert_eq!(
        map3.get("LIBSQL_PATH").map(String::as_str),
        Some("/home/user/.steward/steward.db"),
        "LIBSQL_PATH must be preserved after upsert of different key"
    );
    assert_eq!(
...
fn bootstrap_env_handles_special_characters() {
    ...
    let test_cases: &[(&str, &str)] = &[
        // Spaces in values
        ("AGENT_NAME", "my steward agent"),
        // Equals signs in values (e.g., base64 tokens)
        ("API_TOKEN", "dGVzdA=="),
        // Hash characters inside a path would be treated as comments without quoting.
        ("LIBSQL_PATH", "/tmp/steward#workspace/steward.db"),
        // Single quotes inside double-quoted values
        ("GREETING", "it's a test"),
        // Double quotes (must be escaped)
        ("QUOTED_VAL", r#"say "hello" world"#),
        // Backslashes (must be escaped)
        ("WIN_PATH", r"C:\Users\steward\data"),
