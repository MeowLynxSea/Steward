...
//!
//! See: <https://github.com/MeowLynxSea/steward/issues/352> (QA plan, item 1.1)

use steward_core::tools::validate_tool_schema;
use steward_core::tools::{Tool, ToolRegistry};
...
fn json_tool_freeform_data_field_is_valid() {
    ...
    let tool = steward_core::tools::builtin::JsonTool;
    let schema = tool.parameters_schema();
...
fn http_tool_headers_array_is_valid() {
    ...
    let tool = steward_core::tools::builtin::HttpTool::new();
    let schema = tool.parameters_schema();
...
fn time_tool_schema_is_valid() {
    let tool = steward_core::tools::builtin::TimeTool;
    let schema = tool.parameters_schema();
...
fn shell_tool_schema_is_valid() {
    let tool = steward_core::tools::builtin::ShellTool::new();
    let schema = tool.parameters_schema();
