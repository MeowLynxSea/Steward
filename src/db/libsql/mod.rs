...
pub(crate) const ROUTINE_RUN_COLUMNS: &str = "\
    id, routine_id, trigger_type, trigger_detail, trigger_payload, started_at, \
...
/// Parse an optional JSON value from a nullable text column.
...
pub(crate) fn row_to_routine_run_libsql(row: &libsql::Row) -> Result<RoutineRun, DatabaseError> {
    ...
    Ok(RoutineRun {
        ...
        trigger_payload: get_opt_json(row, 4),
