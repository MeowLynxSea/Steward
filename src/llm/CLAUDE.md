...
| `recording.rs` | `RecordingLlm` — trace capture for E2E replay testing (`STEWARD_RECORD_TRACE`) |
...
- If Codex is logged in with API-key mode, Steward uses the standard OpenAI endpoint.
- If Codex is logged in with ChatGPT OAuth mode, Steward routes to the private `chatgpt.com/backend-api/codex` Responses API via `codex_chatgpt.rs`.
...
**Dual auth modes:**
- **Session token** (default): `NEARAI_SESSION_TOKEN=sess_...`, base URL = `https://private.near.ai`. Tokens are persisted to `~/.steward/session.json` (mode 0600) and optionally to the DB `settings` table (`nearai.session_token`). On 401 responses where the body contains "session" + "expired"/"invalid", `NearAiChatProvider` calls `session.handle_auth_failure()` which triggers the interactive OAuth login flow and retries once. Plain `AuthFailed` 401s are not retried.
...
**Auth flow:** Device code OAuth via `auth.openai.com/api/accounts/deviceauth/*` endpoints. On first run, displays a code for the user to enter at a URL. Tokens are persisted to `~/.steward/openai_codex_session.json` (mode 0600) and auto-refreshed before expiry.
...
Raw provider
  ...
  → RecordingLlm            (trace capture; only when STEWARD_RECORD_TRACE is set)
```
...
Set `STEWARD_RECORD_TRACE=1` to enable live trace recording via `RecordingLlm`. Traces are JSON files containing: memory snapshot, HTTP exchanges from tools, and LLM steps (user inputs, text responses, tool call responses). Replay these in E2E tests via `TraceLlm`. Configure output path with `STEWARD_TRACE_OUTPUT` (default: `trace_{timestamp}.json`).
