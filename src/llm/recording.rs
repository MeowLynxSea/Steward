...
//!
//! Enable by setting `STEWARD_RECORD_TRACE=1` at runtime.
...
impl RecordingLlm {
    ...
    ///
    /// - `STEWARD_RECORD_TRACE` — any non-empty value enables recording
    /// - `STEWARD_TRACE_OUTPUT` — file path (default: `./trace_{timestamp}.json`)
    /// - `STEWARD_TRACE_MODEL_NAME` — model_name field (default: `recorded-{inner.model_name()}`)
    pub fn from_env(inner: Arc<dyn LlmProvider>) -> Option<Arc<Self>> {
        let enabled = std::env::var("STEWARD_RECORD_TRACE")
            .ok()
            .filter(|v| !v.is_empty());
        enabled?;

        let output_path = std::env::var("STEWARD_TRACE_OUTPUT")
            .ok()
            .filter(|v| !v.is_empty())
            .map(PathBuf::from)
            .unwrap_or_else(|| {
        ...
        let model_name = std::env::var("STEWARD_TRACE_MODEL_NAME")
            .ok()
            .filter(|v| !v.is_empty())
...
mod tests {
    ...
    fn from_env_returns_none_when_unset() {
        ...
        unsafe { std::env::remove_var("STEWARD_RECORD_TRACE") };
        let stub = Arc::new(StubLlm::new("response"));
