...
impl CodexChatGptProvider {
    ...
    /// Convert Steward messages to Responses API request JSON.
    ...
    /// `"timestamp": ""`). Steward's tool validation treats these as
    ...
    async fn complete_with_tools(
        ...
        let tool_calls: Vec<ToolCall> = result
            ...
            .map(|tc| {
                ...
                // `"timestamp": ""`), which Steward's tool validation rejects.
