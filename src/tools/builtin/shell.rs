//! Shell execution tool for running commands locally.
//!
...
static DANGEROUS_PATTERNS: LazyLock<Vec<&'static str>> = LazyLock::new(|| {
    vec![
        ...
        ".bash_history",
        "id_rsa",
        // PowerShell injection / elevation
        "invoke-expression",
        "iex ",
        " | iex",
        " | invoke-expression",
...
///
/// When executing commands locally, we scrub the environment to
...
static MEDIUM_RISK_PATTERNS: LazyLock<Vec<&'static str>> = LazyLock::new(|| {
    vec![
        ...
        "mv",
        "move",
        // PowerShell mutation equivalents
        "new-item",
        "ni",
        "remove-item",
        "ri",
...
impl ShellTool {
    ...
    pub fn from_workspace(workspace: Arc<crate::workspace::Workspace>) -> Self {
    ...
    > {
        ...
        let cwd = self.resolve_workdir(user_id, workdir).await?;
        ...
        Ok((output, code as i64, workspace_path))
    }
}
...
impl Tool for ShellTool {
    ...
    fn description(&self) -> &str {
        "Execute shell commands. Use for running builds, tests, git operations, and other CLI tasks. \
         Commands run locally in a subprocess with captured output. Long-running commands have a timeout."
    }
    ...
    async fn execute(
        ...
        let (output, exit_code, workspace_path) = self
            .execute_command(command, &ctx.user_id, workdir, timeout, &ctx.extra_env)
            .await?;
...
mod tests {
    ...
    async fn test_env_scrubbing_hides_secrets() {
        ...
        let secret_var = "STEWARD_TEST_SECRET_KEY";
    ...
    async fn test_env_scrubbing_custom_var_hidden() {
        ...
        unsafe { std::env::set_var("STEWARD_QA_TEST_SECRET", "supersecret123") };
        ...
        let output = result.result.get("output").unwrap().as_str().unwrap();
        assert!(
            !output.contains("STEWARD_QA_TEST_SECRET"),
        ...
        // Clean up
        unsafe { std::env::remove_var("STEWARD_QA_TEST_SECRET") };
    }
