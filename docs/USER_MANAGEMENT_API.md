...
DB-backed user management for multi-tenant IronCowork deployments. Covers admin user CRUD, per-user secrets provisioning, self-service profile, API token management, and usage reporting.
...
Provision secrets on behalf of individual users. The primary use case is an application backend (acting as admin) that configures per-user credentials so each user's IronCowork agent can call back to external services.
...
curl -X POST https://ironcowork.example.com/api/admin/users \
...
curl -X PUT https://ironcowork.example.com/api/admin/users/alice-uuid/secrets/app_callback_token \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -d '{"value": "per-user-jwt-for-alice", "provider": "my-app"}'

# Now Alice's IronCowork agent can use the "app_callback_token" secret
