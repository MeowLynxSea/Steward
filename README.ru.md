...
Steward построен на простом принципе: **ваш AI-ассистент должен работать на вас, а не против вас**.

В мире, где системы ИИ становятся все более непрозрачными в вопросах обработки данных и ориентируются на корпоративные интересы, Steward выбирает другой путь:
...
Steward — это AI-ассистент, которому вы действительно можете доверять в личной и профессиональной жизни.
...
- **Desktop-first** — основной сценарий это Tauri-десктоп с потоками чата и локальным выполнением.
- **Песочница Docker** — изолированное выполнение контейнеров с токенами для каждого задания и паттерном «оркестратор/воркер».
- **Tauri IPC** — интерфейс синхронизируется через Tauri IPC и runtime events, а не через HTTP/SSE.
...
- **Динамическое создание инструментов** — опишите, что вам нужно, и Steward создаст это как инструмент WASM.
...
- Rust 1.85+
- Локальная среда с поддержкой libSQL
- Аккаунт NEAR AI (аутентификация через мастер настройки)

## Загрузка и сборка

Посетите [страницу релизов](https://github.com/nearai/steward/releases/), чтобы увидеть последние обновления.

<details>
  <summary>Установка через установщик Windows (Windows)</summary>

Загрузите [Windows Installer](https://github.com/nearai/steward/releases/latest/download/steward-x86_64-pc-windows-msvc.msi) и запустите его.
...
```bash
# База libSQL создается автоматически при первом запуске
# При необходимости путь можно переопределить через LIBSQL_PATH
```

## Конфигурация

Запустите мастер настройки для конфигурации Steward:

```bash
steward onboard
```

Мастер настройки поможет задать локальную конфигурацию, пройти аутентификацию NEAR AI (через браузер OAuth) и настроить шифрование секретов (используя системную связку ключей). Настройки сохраняются в локальной базе данных; базовые переменные (например, `LIBSQL_PATH`, `LLM_BACKEND`) записываются в `~/.steward/.env`, чтобы они были доступны на раннем этапе запуска.

### Альтернативные LLM-провайдеры

Steward по умолчанию использует NEAR AI, но поддерживает множество LLM-провайдеров из коробки.
...
Steward реализует эшелонированную защиту для обеспечения безопасности ваших данных и предотвращения злоупотреблений.
...
- Все данные хранятся локально в вашей базе данных libSQL.
...
| **Desktop UI** | Tauri-интерфейс для сессий, потоков, сообщений, задач и рутин |
...
```bash
# Первоначальная настройка (локальная конфигурация, аутентификация и т.д.)
steward onboard

# Запуск desktop runtime для разработки
cargo run
...
- Основной поддерживаемый пользовательский путь теперь десктопный. Старые channel-specific инструкции считаются миграционным наследием.

## Наследие OpenClaw

Steward — это реализация на Rust, вдохновленная проектом [OpenClaw](https://github.com/openclaw/openclaw). Полную матрицу соответствия функций можно найти в [FEATURE_PARITY.md](FEATURE_PARITY.md).
...
- **libSQL persistence** — встроенное локальное хранилище для desktop-first продукта.
