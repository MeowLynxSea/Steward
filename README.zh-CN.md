...
Steward 不是给现有 Coding CLI 套一个 GUI，也不再以“预定义工作流驱动”作为产品中心。
...
- **桌面优先**：用 Tauri 提供通知、托盘、拖放等原生能力，并以 Tauri IPC 与原生事件作为桌面主传输层。
...
仓库当前已经围绕 Steward 的产品方向完成收口，正在补齐打包与终端用户文档。
...
- PostgreSQL 假设
- 将 Steward 描述为“预定义工作流产品”的文档

新的核心对象：

- 持久化 agent 会话
- 会话内以 thread 为核心的聊天执行单元
- 附着在线程上的 run / task 执行记录
- Ask/Yolo 批准检查点
- 可选的后台 routine
- 基于 Svelte + Tauri IPC 的桌面 UI
...
- **thread 驱动的聊天执行**：每个 session 里的实际对话与执行载体
...
Steward 会把原有的纵深防御继续应用到桌面自主执行场景：
