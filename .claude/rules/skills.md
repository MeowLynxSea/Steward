...
|-------------|--------|-------------|
| **Trusted** | User-placed in `~/.steward/skills/` or workspace `skills/` | All tools available to the agent |
| **Installed** | Downloaded from ClawHub registry (`~/.steward/installed_skills/`) | Read-only tools only (no shell, file write, HTTP) |
