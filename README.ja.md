...
<p align="center">
  ...
  <a href="https://www.reddit.com/r/stewardAI/"><img src="https://img.shields.io/badge/Reddit-r%2FstewardAI-FF4500?style=flat&logo=reddit&logoColor=white" alt="Reddit: r/stewardAI" /></a>
</p>
...
Stewardはシンプルな原則に基づいて構築されています：**あなたのAIアシスタントは、あなたのために働くべきであり、あなたに不利益をもたらすべきではありません。**

AIシステムがデータの取り扱いについて不透明になり、企業の利益に沿って調整されることが増えている世界で、Stewardは異なるアプローチを取ります：
...
Stewardは、個人生活にも仕事にも本当に信頼できるAIアシスタントです。
...
- **デスクトップ優先** - TauriデスクトップUIを中心に、スレッド駆動の会話とローカル実行を提供
- **Dockerサンドボックス** - ジョブごとのトークンとオーケストレーター/ワーカーパターンによる隔離されたコンテナ実行
- **Tauri IPC** - デスクトップUIはHTTP/SSEではなくTauri IPCとランタイムイベントで同期
...
- **動的ツール構築** - 必要なものを説明すると、StewardがWASMツールとして構築
...
最新のアップデートは[リリースページ](https://github.com/nearai/steward/releases/)をご覧ください。

<details>
  <summary>Windowsインストーラーでインストール（Windows）</summary>

[Windowsインストーラー](https://github.com/nearai/steward/releases/latest/download/steward-x86_64-pc-windows-msvc.msi)をダウンロードして実行してください。
...
```sh
irm https://github.com/nearai/steward/releases/latest/download/steward-installer.ps1 | iex
```
...
```sh
curl --proto '=https' --tlsv1.2 -LsSf https://github.com/nearai/steward/releases/latest/download/steward-installer.sh | sh
```
</details>
...
```bash
# リポジトリをクローン
git clone https://github.com/nearai/steward.git
cd steward
...
```bash
# libSQL データベースは初回起動時に自動作成されます
# 必要に応じて LIBSQL_PATH で保存先を上書きできます
```

## 設定

セットアップウィザードを実行してStewardを設定します：

```bash
steward onboard
```

ウィザードは、ローカル設定、NEAR AI認証（ブラウザOAuth経由）、シークレットの暗号化（システムキーチェーンを使用）を処理します。設定はローカルデータベースに永続化されます。ブートストラップ変数（例：`LIBSQL_PATH`、`LLM_BACKEND`）は、起動初期に利用できるよう`~/.steward/.env`に書き込まれます。

### 代替LLMプロバイダー

StewardはデフォルトでNEAR AIを使用しますが、多くのLLMプロバイダーをすぐに利用できます。組み込みプロバイダーには**Anthropic**、**OpenAI**、**Google Gemini**、**MiniMax**、**Mistral**、**Ollama**（ローカル）が含まれます。**OpenRouter**（300以上のモデル）、**Together AI**、**Fireworks AI**、セルフホストサーバー（**vLLM**、**LiteLLM**）などのOpenAI互換サービスもサポートされています。
...
Stewardは、データを保護し悪用を防ぐために多層防御を実装しています。
...
- すべてのデータはローカルの libSQL データベースに保存
...
| **デスクトップUI** | セッション一覧、スレッド、メッセージ、ジョブ状態、ルーティンを表示するTauriフロントエンド |
...
# デバッグログ付き
RUST_LOG=steward=debug cargo run
```
...
- デスクトップフローが一次経路です。旧チャネル系セットアップ文書は移行対象として扱ってください。

## OpenClawの系譜

Stewardは[OpenClaw](https://github.com/openclaw/openclaw)にインスパイアされたRust再実装です。完全な対応表は[FEATURE_PARITY.md](FEATURE_PARITY.md)をご覧ください。

主な違い：

- **Rust vs TypeScript** - ネイティブパフォーマンス、メモリ安全性、シングルバイナリ
- **WASMサンドボックス vs Docker** - 軽量、機能ベースのセキュリティ
- **libSQL 永続化** - ローカルファーストな埋め込みストレージ
