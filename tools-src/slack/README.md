...
A standalone WASM component that provides Slack integration for Steward. This serves as both a functional tool and a template for building custom WASM tools.
...
```bash
mkdir -p ~/.steward/tools
cp target/wasm32-wasip2/release/slack_tool.wasm ~/.steward/tools/slack.wasm
cp slack.capabilities.json ~/.steward/tools/
```
...
```bash
steward secret set slack_bot_token "xoxb-your-token-here"
```
...
```bash
steward secret set slack_bot_token "xoxb-..."
```
